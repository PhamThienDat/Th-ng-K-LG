...
if(isset($_REQUEST['editId']) and $_REQUEST['editId']!=""){
	$row	=	$db->getAllRecords('member','*',' AND ID="'.$_REQUEST['editId'].'"');
}
...
if($_SERVER["REQUEST_METHOD"]=="POST"){
	...
	if($_POST["username"]==""){
		...
		header('location:'.$_SERVER['PHP_SELF'].'?msg=ux&editId='.$_REQUEST['editId']);
		exit;
	}elseif($_POST["userphone"]==""){
		header('location:'.$_SERVER['PHP_SELF'].'?msg=uz&editId='.$_REQUEST['editId']);
		exit;
	...
	$update	=	$db->update('member',$data,array('ID'=>$editId));
	if($update){
				...
				<div class="col-sm-6">
					...
					<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
						...
						<div class="form-group">
							...
							<input type="text" name="userpass" id="userpass" class="form-control" value="<?php echo $row[0]['pass']; ?>" placeholder="Enter user password" required>
						...
						<div class="form-group">
							...
							<input type="email" class="tel form-control" name="useremail" id="useremail" value="<?php echo $row[0]['email']; ?>" x-autocompletetype="tel" placeholder="Enter user Email" required>
						...
						<div class="form-group">
							...
							<input type="text" class="tel form-control" name="userfullname" id="userfullname" value="<?php echo $row[0]['fullname']; ?>" placeholder="Enter user FullName" required>
						...
						<div class="form-group">
							...
							<input type="tel" maxlength="12" class="tel form-control" name="userphone" id="userphone" value="<?php echo $row[0]['sodt']; ?>" x-autocompletetype="tel" placeholder="Enter user Phone" required>
                     ...
                     Min="0" name="userlever" id="userlever" value="<?php echo $row[0]['loai']; ?>" placeholder="Admin=1, Member =0" required>
