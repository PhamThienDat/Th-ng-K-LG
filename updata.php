...
if(isset($_POST["ID"]) && !empty($_POST["ID"])){
   ...
   if(empty($input_Mien)){
      ...
      if(empty($input_Thang)){
       ...
       //Nếu giá trị rỗng thì in ra Mời nhập lại miền bắc hoặc nam
         ...
         && empty($ThoiGian_err) && empty($Nam_err) && empty($Thang_err)){
             $sql ="UPDATE thongke SET Mien=?,MaKH=?,MaCH=?,TenCH=?,Vung=?,Kieu=?,Seri=?,Loai=?,Model=?,ModelSu=?,GiaTien=?,SoLuong=?,
             TienThuong=?,BillCode=?,BillName=?,ThoiGian=?,Nam=?,Thang=? WHERE ID=?";

             if($stmt = mysqli_prepare($mysqli,$sql)){
              $stmt->bind_param("sssssssssssssssssss",$Mot,$Hai,$Ba,$Bon,$SoNam,$Sau,$Bay,$Tam,$Chin,$Muoi,
              $MuoiMot,$MuoiHai,$MuoiBa,$MuoiBon,$MuoiNam,$MuoiSau,$MuoiBay,$MuoiTam,$MuoiChin);
    ...
    if(isset($_GET["ID"]) && !empty(trim($_GET["ID"]))){
        ...
        $sql="SELECT *FROM thongke WHERE ID=?";
        if($stmt= mysqli_prepare($mysqli,$sql)){
           ...
           if(mysqli_stmt_execute($stmt)){
               ...
               $TenCHCre = $row["TenCH"];
                   ...
                   $LoaiCre = $row["Loai"];
                   $ModelCre = $row["Model"];
                  $ModelSuCre = $row["ModelSu"];
                   $GiaTienCre = $row["GiaTien"];
                  $SoLuongCre = $row["SoLuong"];
                  $TienThuongCre = $row["TienThuong"]; 
                  $BillCodeCre = $row["BillCode"]; 
                  $BillNameCre = $row["BillName"]; 
                $ThoiGianCre = $row["ThoiGian"]; 
                   $NamCre = $row["Nam"]; 
                  $ThangCre = $row["Thang"];
               }
              else{
...
<head>
    ...
    <link rel="stylesheet" href="css/uda.css">
    <title>Create</title>
    <style type="text/css">
      .wrapper{
          width: 500px;
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["REQUEST_URI"]); ?>" method="POST">
                     ...
                     <div class="form-group <?php echo (!empty($BillCode_err)) ? 'has-error' : ''?>">
                       ...
                       <input type="text" name="BillCodeCre" class="form-control" value="<?php echo $BillCodeCre;?>">
                       <span class="help-block"><?php echo $BillCode_err; ?></span>
                     </div>
                     <div class="form-group <?php echo (!empty($BillName_err)) ? 'has-error' : ''?>">
                       <label>Bill to name</label>
                       <input type="text" name="BillNameCre" class="form-control" value="<?php echo $BillNameCre;?>">
                     ...
                     <div class="form-group <?php echo (!empty($ThoiGian_err)) ? 'has-error' : ''?>">
                       ...
                       <input type="text" name="ThoiGianCre" class="form-control" value="<?php echo $ThoiGianCre;?>">
                       <span class="help-block"><?php echo $ThoiGian_err; ?></span>
                     </div>
                     <div class="form-group <?php echo (!empty($Nam_err)) ? 'has-error' : ''?>">
                       <label>Year</label>
                       <input type="text" name="NamCre" class="form-control" value="<?php echo $NamCre;?>">
                       <span class="help-block"><?php echo $Nam_err; ?></span>
                     </div>
                     <div class="form-group <?php echo (!empty($Thang_err)) ? 'has-error' : ''?>">
                       <label>Month</label>
                       <input type="text" name="ThangCre" class="form-control" value="<?php echo $ThangCre;?>">
