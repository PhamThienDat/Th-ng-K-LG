...
<body>
	...
	if(isset($_REQUEST['userpass']) and $_REQUEST['userpass']!=""){
		$condition	.=	' AND pass LIKE "%'.$_REQUEST['useremail'].'%" ';
	}
	if(isset($_REQUEST['useremail']) and $_REQUEST['useremail']!=""){
		$condition	.=	' AND email LIKE "%'.$_REQUEST['userphone'].'%" ';
	}
	if(isset($_REQUEST['userfullname']) and $_REQUEST['userfullname']!=""){
		$condition	.=	' AND fullname LIKE "%'.$_REQUEST['userphone'].'%" ';
	}
	if(isset($_REQUEST['userphone']) and $_REQUEST['userphone']!=""){
		$condition	.=	' AND sodt LIKE "%'.$_REQUEST['userphone'].'%" ';
	}
	if(isset($_REQUEST['userlever']) and $_REQUEST['userlever']!=""){
		$condition	.=	' AND loai LIKE "%'.$_REQUEST['userphone'].'%" ';
	}
	...
	$userData	=	$db->getAllRecords('member','*',$condition,'ORDER BY id DESC');
	?>
	
   	<div class="container">
		
		<div style="margin-top:50px;" class="card">
			<div class="card-header"> <a href="homeadmin.php" class="float-left btn btn-dark btn-sm">Home</a> <a href="add-users.php" class="float-right btn btn-dark btn-sm"><i class="fa fa-fw fa-plus-circle"></i> Add Users</a></div>
			<div class="card-body">
				<?php
				...
				<tbody>
					...
					<tr>
						...
						<td align="center">
							...
							<a href="delete_user.php?delId=<?php echo $val['ID'];?>" class="text-danger" onClick="return confirm('Are you sure to delete this user?');"><i class="fa fa-fw fa-trash"></i> Delete</a>
						</td>
        ...
        $result = mysqli_query($mysqli,"SELECT count(ID) as total from member");
        ...
        $result = mysqli_query($mysqli, "SELECT * FROM member LIMIT $start, $limit");
